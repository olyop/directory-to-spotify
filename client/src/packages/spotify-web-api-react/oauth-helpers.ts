...
export const hasValidStoredAccessToken = () => {
	const accessToken = retrieveStoredAccessToken();
	...
	const isValid =
		...
		"tokenType" in accessToken &&
		typeof accessToken.tokenType === "string" &&
		accessToken.tokenType.length > 0 &&
		"expiresAt" in accessToken &&
		typeof accessToken.expiresAt === "number" &&
		accessToken.expiresAt > Date.now() &&
		"refreshToken" in accessToken &&
		typeof accessToken.refreshToken === "string" &&
		accessToken.refreshToken.length > 0 &&
		"scope" in accessToken &&
		typeof accessToken.scope === "string" &&
		accessToken.scope.length > 0;
...
export const convertUserData = (data: Record<string, unknown>, options: SpotifyInternalOptions): SpotifyUser => {
