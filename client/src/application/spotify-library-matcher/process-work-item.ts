...
export const matchWorkItem = async (spotify: SpotifyWebApiClient, workItem: WorkItem): Promise<WorkItemMatch> => {
...
export async function search(client: SpotifyWebApiClient, query: string) {
...
function prepareString(value: MatchValue) {
	return removeDoubleWhiteSpace(removeSymbols(removeBlackListedWords(value?.toLowerCase().trim() ?? "")));
}
